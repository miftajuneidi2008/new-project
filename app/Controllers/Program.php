...
class Program extends BaseController
{
    public function index()
    {
        ...
        $data['program'] = $model->getNewsWithDetails();
    ...
    public function new()
    {
        $category_Model = new ProgramCategoryModel();
        $data['program_categories'] = $category_Model->findAll();
        return view('admin/program/new/index', $data);
    }
    ...
    public function create()
    {
        ...
        $rules = [
            ...
            'program_category_id' => [
                ...
                // This rule ensures the submitted ID actually exists in the 'program_categories' table.
                'rules' => 'required|is_not_unique[program_categories.id]',
                'errors' => [
                    'required' => 'You must select a Program category.',
                    'is_not_unique' => 'The selected program category is not valid.'
                ]
            ],
            'description' => [
                'label' => 'Description',
        ...
        if ($img->isValid() && !$img->hasMoved()) {
            ...
            $data = [
                ...
                'user_id' => $userId,
                'category_id' => $this->request->getPost('program_category_id'),
                'photo' => $newName,
                'description' => $cleanHTML,
            ];

            if ($program_model->save($data)) {
                return redirect()->to('/admin/program/new')->with('message', 'Program  created successfully.');
            }
        }
        // This case is for database errors, not validation errors.
        return redirect()->back()->withInput()->with('errors', ['database' => 'Failed to save the Program to the database.']);
    ...
    public function edit($id = null)
    {
        ...
        $data['program_categories'] = $categoryModel->findAll();
        $data['program'] = $model->find($id);

        return view('admin/program/edit/index', $data);
    }
    ...
    public function update($id = null)
    {
        ...
        if ($model->update($id, $data)) {
            ...
            return redirect()->to('/admin/program/')->with('message', 'Program category updated successfully.');
        } else {
