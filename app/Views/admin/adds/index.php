...
<div class="container-fluid px-4 mx-2">
    ...
    <h2 class="h4 mb-3">የአሁኑ ማስታወቂያ</h2>
        ...
        <?php if (!empty($adds)): ?>
            ...
            <div class="col">
                <div class="card" style="width: 24rem;">
                    <div class="card-body">
                        <img src="<?= base_url('images/' . $adds[0]['photo']) ?>" class="card-img-top" alt="አርማ"
                            style="width=300px; height: 200px; object-fit: cover;margin-bottom:10px;">
                                  <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal"
                            data-bs-target="#editAddsModal" data-adds-id="<?= esc($adds[0]['id']) ?>"
                            data-adds-content="<?= esc($adds[0]['photo']) ?>">
                            አሻሽል
                        </button>
                           <form action="<?= base_url('admin/adds/delete/' . esc($adds[0]['id'])) ?>" method="post" class="d-inline"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editLinksModalLabel">ማስታወቂያ አሻሽል</h5>
                    ...
                    <form id="editLogoForm" action="" method="post" enctype="multipart/form-data">
                        <div class="modal-body">
                            <?= csrf_field() ?>
                            <div class="mb-3">
                                <label for="editCommentText" class="form-label">ማስታወቂያ:</label>
...
<div class="card shadow-lg mb-4 col-12 col-md-8">
    <div class="card-body">
        ...
        <form action="/admin/adds/" method="post" enctype="multipart/form-data">
            <?= csrf_field() ?>

            <div class="mb-3">
                <label for="description" class="form-label fs-5">
                    ከዚህ በታች የማስታወቂያ ምስል ያክሉ</label>
...
<script>
    ...
    editLogoModal.addEventListener('show.bs.modal', function (event) {
        ...
        modalForm.action = "<?= base_url('admin/adds/update/') ?>" + logoId;
    });
</script>
<?= $this->endSection() ?>